...
import verification
import sklearn
from combinedloss import Intraloss,Interloss
import datetime
...
def parse_args():
  ...
  # general
  parser.add_argument('--data-dir', default='D:\\han\\insightface-master\\insightface-master\\datasets\\protocal_2', help='training set directory')
  parser.add_argument('--prefix', default='D:\\han\\insightface-master\\insightface-master\\models\\p2_517\\model', help='directory to save model.')
  parser.add_argument('--pretrained', default='D:\\han\\insightface-master\\insightface-master\\models\\modeltest\\model', help='pretrained model to load')
  ...
  parser.add_argument('--loss-type', type=int, default=8, help='loss type')
  parser.add_argument('--verbose', type=int, default=10000, help='do verification testing and model saving every verbose batches')
  parser.add_argument('--max-steps', type=int, default=0, help='max training batches')
  parser.add_argument('--pretrained-epoch', type=int, default=0, help='pretrained epoch to load')
  parser.add_argument('--end-epoch', type=int, default=5000, help='training epoch size.')
  ...
  parser.add_argument('--version-output', type=str, default='GDC', help='network embedding output config')
  ...
  parser.add_argument('--lr-steps', type=str, default='80000,150000,200000', help='steps of lr changing')
  ...
  parser.add_argument('--margin-a', type=float, default=0.9, help='')
  parser.add_argument('--margin-b', type=float, default=0.15, help='')
  ...
  parser.add_argument('--target', type=str, default=['dfw'], help='verification targets')
...
def build_log_dir(name):
  ...
  # Output csv header
  with open(log_path + '/record.csv', 'a') as f:
      f.write(
          'Eopch, acc, loss\n')
...
def get_symbol(args, arg_params, aux_params):##symbol是定義好的網路，這一段在定義網路
  data_shape = (args.image_channel,args.image_h,args.image_w)##[3,112,112]
  ...
  gt_label = all_label
  extra_loss = None
  _weight = mx.symbol.Variable("fc7_weight", shape=(args.num_classes, args.emb_size), lr_mult=args.fc7_lr_mult, wd_mult=args.fc7_wd_mult)#Embedding layer(fc1),fc7(embedding to number of classes)，lr_mult的意義是該層的參數更新的learn rate需要在基礎的base learn rate上乘以該參數，即lr=lr_base*lr_mult,wd_mult是指的对应的weight decay term是设置的wd的wd_mult倍
  if args.loss_type==0: #softmax
    if args.fc7_no_bias:
      fc7 = mx.sym.FullyConnected(data=embedding, weight = _weight, no_bias = True, num_hidden=args.num_classes, name='fc7')#全連接層後，loss層
    else:
      _bias = mx.symbol.Variable('fc7_bias', lr_mult=2.0, wd_mult=0.0)
      fc7 = mx.sym.FullyConnected(data=embedding, weight = _weight, bias = _bias, num_hidden=args.num_classes, name='fc7')##The f7 layer is the classifier layer in the face model.
  ...
  elif args.loss_type==8:##combined intra+inter loss自訂
    intraloss, fc7 = Intraloss(args,_weight,embedding,gt_label)
    interloss = Interloss(args,_weight,embedding,gt_label)
    #intraloss = mx.sym.MakeLoss(intraloss, name='intraloss', grad_scale = args.margin_b)
    
    combinedloss = intraloss.sum() + interloss.sum()
    combinedloss = mx.sym.MakeLoss(combinedloss, name='combinedloss', grad_scale = args.margin_b)
    #print('combinedloss:',combinedloss)
    
  out_list = [mx.symbol.BlockGrad(embedding)]##回傳零梯度，不對embedding求導，僅獲取embedding值
...
def train_net(args):
    ...
    # else:
      ...
      #vec = args.pretrained.split(',')
        ...
        # mx.model.load_checkpoint(args.pretrained, args.pretrained_epoch)
        #_, arg_params, aux_params = mx.model.load_checkpoint(vec[0], int(vec[1]))
        _, arg_params, aux_params = mx.model.load_checkpoint(args.pretrained,args.pretrained_epoch)
        sym, arg_params, aux_params = get_symbol(args, arg_params, aux_params)

    log_path = build_log_dir(args.name)

    #label_name = 'softmax_label'
    #label_shape = (args.batch_size,)
    model = mx.mod.Module(##建立模型
        context       = ctx,
        symbol        = sym,##這裡的sym是get symbol()return的out
    )
    val_dataiter = None
    ...
    if args.ce_loss:
      ...
      # print('__DEBUG__')
      print(mx.metric.create(metric2))
      # print('__DEBUG__')
    ...
    def ver_test(nbatch):##應該是用驗證集測試模型
      results = []
    ...
    if len(args.lr_steps)==0:
      ...
      if args.loss_type>=1 and args.loss_type<=8:
    ...
    def _batch_callback(param):
      ...
      if global_step[0] % (args.num_classes//args.per_batch_size) ==0:
        ...
        print(param.epoch)
        with open(log_path + '/record.csv', 'a') as f:
          f.write('%d, %.15f, %.15f\n' % (param.epoch, metric1.get()[1], metric2.get()[1]))
        print('__DEBUG__')
