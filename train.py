...
def parse_args():
  ...
  parser.add_argument('--wd', type=float, default=0.0005, help='weight decay')##Weight-Decay 其實就是我們最常說的 L1 / L2 regularization
...
def train_net(args):
    ...
    assert len(ppatch)==5 ##assert用來除錯，如果上一行不照assert的條件，會提示錯誤，例len(ppatch)=4，出錯


    os.environ['BETA'] = str(args.beta)##For SphereFace(jointed training with softmax)
    data_dir_list = args.data_dir.split(',')##訓練集路徑
    if args.loss_type!=12 and args.loss_type!=13:##不是triplet loss 和triplet loss margin
      assert len(data_dir_list)==1
    data_dir = data_dir_list[0]##可能是訓練集的根資料夾
    args.use_val = False
    path_imgrec = None
    path_imglist = None
    val_rec = None
    prop = face_image.load_property(data_dir)##回傳ㄧ個dict {'num_class':8277,'image_size':112,112}
    ...
    if args.loss_type==1 and args.num_classes>20000:##sphere
    ...
    if args.loss_type<9:
      ...
      if args.images_per_identity==0:
        if args.loss_type==11:
          ...
          assert args.per_batch_size%3==0##triplet loss
      assert args.images_per_identity>=2##因為pair
      args.per_identities = int(args.per_batch_size/args.images_per_identity)

    print('Called with argument:', args)##把所有參數印出來
    ...
    if len(args.pretrained)==0:
      arg_params = None
      aux_params = None
      sym, arg_params, aux_params = get_symbol(args, arg_params, aux_params)##建模從這開始??
    else:##有pretrain model
      vec = args.pretrained.split(',')
      print('loading', vec)##列出pretrained model的路徑
      _, arg_params, aux_params = mx.model.load_checkpoint(vec[0], int(vec[1]))##def load_checkpoint(prefix, epoch):,Load model checkpoint from file.
